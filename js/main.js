...
function attachHandlers(){ const root=document.getElementById('app'); root.querySelectorAll('[data-action]').forEach(el=>el.addEventListener('click',()=>actions.handleAction(el.dataset.action))); root.querySelectorAll('[data-open]').forEach(el=>el.addEventListener('click',()=>openPresupuesto(el.dataset.open))); const si=root.querySelector('[data-search]'); if(si) si.addEventListener('input',()=>{ui.search=si.value; render();}); root.querySelectorAll('[data-field]').forEach(el=>el.addEventListener('input',()=>{const path=el.dataset.field; let val=el.value; if(el.type==='number') val=val===''?'':parseFloat(val); setPath(ui.draft,path,val); updateTotalCard(); })); root.querySelectorAll('[data-line-field]').forEach(el=>el.addEventListener('input',()=>{const idx=parseInt(el.dataset.idx,10); const field=el.dataset.lineField; let val=el.value; if(field==='precio') val=val===''?'':parseFloat(val); if(!ui.draft.lineas[idx]) ui.draft.lineas[idx]={descripcion:'',precio:''}; ui.draft.lineas[idx][field]=val; updateTotalCard(); })); root.querySelectorAll('[data-remove-line]').forEach(el=>el.addEventListener('click',()=>{ui.draft.lineas.splice(parseInt(el.dataset.removeLine,10),1); render();})); root.querySelectorAll('[data-setting]').forEach(el=>{el.addEventListener('change',actions.saveSettings); el.addEventListener('blur',actions.saveSettings);}); }
